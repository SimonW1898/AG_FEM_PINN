...
To derive the weak formulation of the time dependent Schrödinger equation, we multiply the equation by a test function $v(x,t) \in H_0^1(\Omega; \mathbb{C})$ and integrate over the domain $\Omega$. 
...
where $v$ is called the test function and $\bar{v}$ is its complex conjugate, which comes from the inner product in the Sobolev space $H_0^1(\Omega; \mathbb{C})$.
...
\subsection*{Weak Formulation with Time Integration}
